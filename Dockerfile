...
RUN apt-get update && \
    apt-get install --no-install-recommends -y gfortran-8 libfftw3-dev python3-dev curl git tar wget build-essential cmake time nano && \
    apt-get clean && \
...
RUN python -m pip install cdsapi
